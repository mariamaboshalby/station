...
<div class="container-fluid py-5" dir="rtl" style="font-family: 'Tajawal', sans-serif;">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        ...
        <form action="{{ route('treasury.index') }}" method="GET" class="d-flex align-items-center bg-white p-2 rounded-pill shadow-sm">
            <input type="date" name="date" value="{{ $date }}" class="form-control border-0 bg-transparent fw-bold" onchange="this.form.submit()">
    ...
    <div class="row g-4 mb-5">
        ...
        <div class="col-md-3">
            <div class="card border-0 shadow-sm rounded-4 h-100 overflow-hidden position-relative group-hover bg-warning text-white">
                <div class="card-body p-4 position-relative z-1">
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-header bg-white border-0 p-4 pb-0">
                    <h5 class="fw-bold text-dark mb-0"><i class="fas fa-plus-circle text-primary me-2"></i>إضافة حركة جديدة</h5>
                </div>
                    ...
                    <form action="{{ route('treasury.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="btn-group w-100" role="group">
                                ...
                                <label class="btn btn-outline-success fw-bold py-2" for="type_income">📥 إيراد (دخول)</label>

                                <input type="radio" class="btn-check" name="type" id="type_expense" value="expense">
                                <label class="btn btn-outline-danger fw-bold py-2" for="type_expense">📤 مصروف (خروج)</label>
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="form-label text-secondary small fw-bold">تاريخ الحركة</label>
                            <input type="date" name="transaction_date" value="{{ date('Y-m-d') }}" class="form-control bg-light border-0 fw-bold">
                        </div>

                        <div class="mb-3">
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                <label class="form-label text-secondary small fw-bold mb-0">المبلغ</label>
                                <button type="button" id="toggleCalcBtn" class="btn btn-sm text-primary p-0 small fw-bold" style="font-size: 0.8rem;">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="category" list="categories" class="form-control bg-light border-0 text-start" placeholder="مثال: مبيعات بنزين، موردين..." required>
                        ...
                        {{-- 🧮 Fuel Calculator --}}
                        <div id="fuelCalc" class="mb-3 p-3 bg-warning-subtle rounded-3 d-none border border-warning-subtle">
                            <h6 class="text-dark fw-bold small mb-2"><i class="fas fa-gas-pump me-1"></i> حاسبة تكلفة الوقود (إضافة لترات)</h6>
                            <div class="row g-2">
                                <div class="col-6">
                                    <label class="small text-muted fw-bold">عدد اللترات</label>
                                    <input type="number" id="calc_liters" class="form-control form-control-sm border-0 shadow-sm" placeholder="مثال: 1000">
                                </div>
                                <div class="col-6">
                                    <label class="small text-muted fw-bold">سعر الشراء / لتر</label>
                                    <input type="number" id="calc_price" step="0.01" class="form-control form-control-sm border-0 shadow-sm" placeholder="مثال: 11.5">
                                </div>
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" id="description" class="form-control bg-light border-0" rows="2" placeholder="تفاصيل إضافية..."></textarea>
                        </div>

                        <button type="submit" class="btn btn-dark w-100 py-2 fw-bold rounded-3 shadow-hover transform-scale">
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm rounded-4 h-100">
                <div class="card-header bg-white border-0 p-4 pb-0 d-flex justify-content-between align-items-center">
                    <h5 class="fw-bold text-dark mb-0"><i class="fas fa-list-ul text-secondary me-2"></i>سجل حركات اليوم ({{ $date }})</h5>
                </div>
                            ...
                            <tbody>
                                @forelse($todayTransactions as $trx)
                                    <tr class="border-bottom border-light">
                                        ...
                                        <td class="px-3 text-muted small">{{ $trx->description ?? '-' }}</td>
                                        <td class="px-3 text-end fw-bold {{ $trx->type == 'income' ? 'text-success' : 'text-danger' }}" dir="ltr">
                                            {{ number_format($trx->amount, 2) }}
                                        </td>
                                        <td class="px-3 text-center">
                                            <form action="{{ route('treasury.destroy', $trx->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من الحذف؟');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-link text-danger p-0 opacity-50 hover-opacity-100">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function checkCategory() {
                ...
                if(val.includes('شراء') || val.includes('وقود') || val.includes('بنزين') || val.includes('سولار')) {
    ...
    <style>
        ...
        .bg-gradient-primary { background: linear-gradient(135deg, #0d6efd, #0a58ca) !important; }
        .bg-success-subtle { background-color: #d1e7dd; }
        .bg-danger-subtle { background-color: #f8d7da; }
        .shadow-hover:hover { box-shadow: 0 10px 20px rgba(0,0,0,0.1) !important; transform: translateY(-2px); transition: all 0.3s; }
