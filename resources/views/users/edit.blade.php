                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="phone" value="{{ old('phone', $user->phone) }}"
                                class="form-control" required pattern="[0-9]{11}" maxlength="11">
                        </div>

                        <div class="col-md-6">
                            <label class="form-label fw-bold"><i class="fa fa-key me-1"></i> كلمة المرور الجديدة
                                (اختياري)</label>
                        ...
                        <div class="col-12 mt-3">
                            ...
                            <div class="row">
                                @foreach ($fuels as $fuel)
                                    @php
                                        $price = $user->fuelPrices->where('fuel_id', $fuel->id)->first()?->price;
                                    @endphp
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <input type="number" step="0.001" name="fuel_prices[{{ $fuel->id }}]"
                                            ...
                                            value="{{ old('fuel_prices.' . $fuel->id, $price) }}">
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        @foreach ($generalPermissions as $category => $permissions)
                            <div class="col-md-4 mb-3">
                                <div class="card shadow-sm mb-4 border-0 rounded-3">
                                    <div
                                        class="card-header bg-light d-flex justify-content-between align-items-center py-2 px-3 rounded-top">
                                        <h6 class="mb-0 fw-bold text-primary">
                                            <i
                                                class="fa {{ $permissionCategories[$category]['icon'] }} me-2 text-secondary"></i>
                                        ...
                                        @foreach ($permissions as $permission)
                                            <div class="form-check mb-2 ms-3">
                                                ...
                                                <label class="form-check-label small fw-semibold"
                                                    for="perm_{{ str_replace(' ', '_', $permission['value']) }}">
                                                    {{ $permission['label'] }}
                                                </label>
                                            </div>
                                        @endforeach
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <div class="card shadow-sm mb-4 border-0 rounded-3">
                                    <div
                                        ...
                                        <h6 class="mb-0 fw-bold text-primary">
                                            <i
                                                class="fa {{ $fuelData['icon'] ?? 'fa-gas-pump' }} me-2 text-secondary"></i>
                                            {{ $fuelData['name'] ?? 'وقود' }}
                                        </h6>
                                    ...
                                    <div class="card-body bg-white">
                                            ...
                                            @foreach ($tanks as $tank)
                                                <div class="tank-section mb-4 p-3 border rounded bg-light-subtle">
                                                    <h6 class="fw-bold mb-3 text-secondary">
                                                        <i class="fa fa-oil-can me-2 text-info"></i> تانك:
                                                        {{ $tank->name }}
                                                        <small class="text-muted">(سعة:
                                                            {{ number_format($tank->capacity, 2) }} لتر)</small>
                                                    </h6>
                                                            ...
                                                            <div class="pump-item mb-2 ms-3">
                                                                <div class="form-check">
                                                                    ...
                                                                    <label class="form-check-label fw-semibold"
                                                                        for="pump_{{ $pump->id }}">
                                                                        <i
                                                                            class="fa fa-tachometer-alt me-1 text-primary"></i>
                                                                        طلمبة: {{ $pump->name }}
                                                                    </label>
                                                                </div>
                                                            </div>
                                                    ...
                                                    @else
                                                        <div class="alert alert-info py-1 mb-0">
                                                            <small><i class="fa fa-info-circle me-1"></i> لا توجد طلمبات
                                                                لهذا التانك</small>
                                                        </div>
                                                    @endif
                                                </div>
                                        ...
                                        @else
                                            <div class="alert alert-warning text-center mb-0">
                                                <i class="fa fa-exclamation-triangle me-2"></i> لا توجد تانكات
                                                {{ $fuelData['name'] ?? '' }}
                                            </div>
                                        @endif
                                    </div>
