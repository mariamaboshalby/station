    ...
    <div class="container-fluid pb-5" dir="rtl" style="font-family: 'Tajawal', sans-serif;">
        ...
        <div class="d-print-none mb-4 d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('reports.expenses') }}" class="btn btn-danger shadow-sm px-4 rounded-pill">
                    <i class="fas fa-arrow-up me-2"></i> تقرير المصروفات
                </a>
                <a href="{{ route('reports.clients') }}" class="btn btn-warning shadow-sm px-4 rounded-pill">
                    <i class="fas fa-users me-2"></i> حسابات العملاء
                </a>
                <div class="dropdown">
                    <button class="btn btn-secondary shadow-sm px-4 rounded-pill dropdown-toggle" type="button"
                    ...
                    <ul class="dropdown-menu text-end">
                        <li>
                            <a class="dropdown-item"
                                href="{{ route('reports.export', array_merge(request()->all(), ['type' => 'pdf'])) }}">
                                <i class="fas fa-file-pdf text-danger me-2"></i> تصدير PDF
                            </a>
                        </li>
                        <li>
                            <a class="dropdown-item"
                                href="{{ route('reports.export', array_merge(request()->all(), ['type' => 'excel'])) }}">
                ...
                <form action="{{ route('reports.index') }}" method="GET" class="row g-3 align-items-end">
                    <div class="col-md-3">
                        ...
                        <select name="payment_type" class="form-select border-0 bg-light py-2 fw-bold text-dark">
                            ...
                            <option value="cash" {{ $paymentType == 'cash' ? 'selected' : '' }}>💵 نقدي (كاش + مصروفات)
                            </option>
            ...
            <div class="col-md-3">
                <div
                    ...
                    <div class="card-body p-4 position-relative z-1">
                        ...
                        <h2 class="fw-bold text-white mb-0 display-6" dir="ltr">{{ number_format($totalExpense, 2) }}
                        </h2>
                        <span class="text-white-50 small">جنية مصري</span>
                    </div>
                    <div class="position-absolute bottom-0 end-0 p-3 opacity-25">
            ...
            <div class="col-md-3">
                <div
                    ...
                    <div class="card-body p-4 position-relative z-1">
                        ...
                        <h2 class="fw-bold text-white mb-0 display-6" dir="ltr">{{ number_format($netProfit, 2) }}
                        </h2>
                        <span class="text-white-50 small">جنية مصري</span>
                    </div>
                    <div class="position-absolute bottom-0 end-0 p-3 opacity-25">
        ...
        <div class="row g-4 mb-5">
            ...
            <div class="col-md-6">
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-header bg-transparent border-0 p-4 pb-0">
                        <h5 class="fw-bold text-dark mb-0"><i class="fas fa-chart-pie text-success me-2"></i>تفاصيل
                            الإيرادات</h5>
                    </div>
                                ...
                                <tbody>
                                    @forelse($revenueByType as $type => $amount)
                                        <tr>
                                            ...
                                            <td class="text-end fw-bold text-success" dir="ltr">
            ...
            <div class="col-md-6">
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-header bg-transparent border-0 p-4 pb-0">
                        <h5 class="fw-bold text-dark mb-0"><i class="fas fa-chart-bar text-danger me-2"></i>تفاصيل
                            المصروفات</h5>
                    </div>
                    <div class="card-body p-4">
                        <div class="table-responsive">
                                ...
                                <tbody>
                                    @forelse($expenseByCategory as $cat => $amount)
                                        <tr>
                                            ...
                                            <td class="text-end fw-bold text-danger" dir="ltr">
                                                {{ number_format($amount, 2) }}</td>
                                        </tr>
                                    @empty
                                        <tr>
        ...
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
            <div class="card-header bg-white border-bottom p-4 d-flex justify-content-between align-items-center">
                <h5 class="fw-bold m-0 text-dark"><i class="fas fa-list-ul me-2 text-primary"></i>سجل المعاملات اليومي
                </h5>
                        ...
                        @forelse($reportData as $row)
                            <tr class="border-bottom border-light">
                                <td class="px-4 text-muted fw-bold small">
                                    {{ $row['date']->format('Y-m-d') }} <span
                                        class="d-block fw-normal text-secondary mt-1">{{ $row['date']->format('h:i A') }}</span>
                                </td>
                                <td class="px-4">
                                    <span
                                        class="badge rounded-pill px-3 py-2 {{ $row['is_revenue'] ? 'bg-success-subtle text-success' : 'bg-danger-subtle text-danger' }}">
                                ...
                                <td class="px-4 text-end fw-bold {{ $row['is_revenue'] ? 'text-success' : 'text-danger' }}"
                                    dir="ltr">
